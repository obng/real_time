...
function Notifications() {
  ...
  // 실제로는 auth context 등에서 ownerId를 받아와야 합니다.
  ...
  useEffect(() => {
    async function fetchNotifications() {
      try {
        const res = await fetch(`/api/owner/notification/${ownerId}`);
  ...
  }, []); // ownerId가 고정이라면 빈 배열로 호출 1회만

  // 2. SSE(실시간 알림) 구독
  useEffect(() => {
    const eventSource = new EventSource(`/api/owner/notification/${ownerId}/subscribe`);
